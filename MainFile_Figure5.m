%%=============================================================
%The file is used to generates the data applied in Fig. 5 of the paper:
%
...
SE_LMMSE_Distributed_total = zeros(K,nbrOfSetups,length(M_total));
SE_OBE_Distributed_Monte_total = zeros(K,nbrOfSetups,length(M_total));
SE_MR_Distributed_Monte_total = zeros(K,nbrOfSetups,length(M_total));

SE_LMMSE_LSFD_total = zeros(K,nbrOfSetups,length(M_total));
SE_OBE_LSFD_Monte_total = zeros(K,nbrOfSetups,length(M_total));
SE_MR_LSFD_Monte_total = zeros(K,nbrOfSetups,length(M_total));
...
for m = 1:length(M_total)
    ...
    for i = 1:nbrOfSetups
        ...
        [SE_MR_LSFD_Monte,SE_MR_Distributed_Monte] = functionComputeSE_Distributed_Monte(H,Hhat_MMSE,tau_c,tau_p,nbrOfRealizations,N,K,M,pv);


        SE_LMMSE_Distributed_total(:,i,m) = SE_LMMSE_Distributed;
        SE_OBE_Distributed_Monte_total(:,i,m) = SE_OBE_Distributed_Monte;
        SE_MR_Distributed_Monte_total(:,i,m) = SE_MR_Distributed_Monte;

        SE_LMMSE_LSFD_total(:,i,m) = SE_LMMSE_LSFD;
        SE_OBE_LSFD_Monte_total(:,i,m) = SE_OBE_LSFD_Monte;
        SE_MR_LSFD_Monte_total(:,i,m) = SE_MR_LSFD_Monte;


        disp([num2str(i) ' setups out of ' num2str(nbrOfSetups)]); 

        clear V_MMSE_Combining_Distributed SE_LMMSE_Distributed V_OBE_Combining_Distributed W_OBE_matrix_Distributed 
