function [V_OBE_Combining_Distributed_Local,W_OBE_matrix] = functionOBE_Combining_Distributed_Local_wo_PS(G_LoS_eff,Ghat_MMSE,Rhat,Phi,R,Pset,M,N,K,p,tau_p,nbrOfRealizations)
%%=============================================================
%The file is used to design DL-OBE combining vectors without random phase-shifts based on closed-form OBE matrices of the paper:
%
    ...
    for k = 1:K
        ...
        for l = 1:K
            ...
            if any(l == Pset(:,k))
                ...
                Gamma_mk = Gamma_mk + p(l)*sqrt(p(k)*p(l))*tau_p*gbar_mlk*rwave_mlk' + p(l)*sqrt(p(k)*p(l))*tau_p*rwave_mlk*gbar_mlk' + p(l)*p(k)*p(l)*tau_p^2*(rwave_mlk*rwave_mlk');
