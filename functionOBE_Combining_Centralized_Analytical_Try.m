function [SINR,V_OBE_Combining_Centralized,W_OBE_matrix,Gamma1,Gamma2,Gamma3] = functionOBE_Combining_Centralized_Analytical_Try(G_LoS_eff,Ghat_MMSE,Rhat,Phi,R,Pset,M,N,K,p,tau_p,nbrOfRealizations,Term1)

% The file designs centralized OBE combining vectors 
...
Gamma1 = zeros((M*N)^2,(M*N)^2,K);
Gamma2 = zeros((M*N)^2,(M*N)^2,K);
Gamma3 = zeros(M*N,M*N,K);
...
%     Gamma = zeros((M*N)^2,(M*N)^2);
    ...
    Gamma1(:,:,k) = Gamma1(:,:,k) + p(l)*kron(Gbar_kl(:,:,k,k).',Gbar_kl(:,:,l,l)) + p(l)*kron(Gbar_kl(:,:,k,k).',R_blk(:,:,l)) + p(l)*kron(Rhat_blk(:,:,k).',Gbar_kl(:,:,l,l))...
    ...
    if any(l == Pset(:,k))
        ...
        Gamma1(:,:,k) = Gamma1(:,:,k) + p(l)*sqrt(p(k)*p(l))*tau_p*gbar_lk(:)*rwave_lk(:)' + p(l)*sqrt(p(k)*p(l))*tau_p*rwave_lk(:)*gbar_lk(:)'...
    ...
    Gamma2(:,:,k) = rbar_k(:)*rbar_k(:)';
    Gamma3(:,:,k) = Rbar(:,:,k);



    Gamma = Gamma1(:,:,k) - p(k)*rbar_k(:)*rbar_k(:)' + kron(Rbar(:,:,k).',eye(M*N));
    ...
    SINR(k) = p(k)*rbar_k(:)'*W_OBE_vector;
