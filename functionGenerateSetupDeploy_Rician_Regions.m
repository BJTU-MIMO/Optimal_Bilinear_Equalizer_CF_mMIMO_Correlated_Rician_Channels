function [R_AP,HMean_Withoutphase] = functionGenerateSetupDeploy_Rician_Regions(M,K,N,nbrOfSetups,correlatedShadowing,Rician_Region)
%%=============================================================
%The file is used to generate the system setup with different Rician distances of the paper:
%
...
R_AP = zeros(N,N,M,K,length(Rician_Region),nbrOfSetups);
channelGain_LoS = zeros(M,K,length(Rician_Region),nbrOfSetups);
channelGain_NLoS = zeros(M,K,length(Rician_Region),nbrOfSetups);
HMean_Withoutphase = zeros(M*N,K,length(Rician_Region),nbrOfSetups);
...
channelGaindB = zeros(M,K,length(Rician_Region));
channelGain = zeros(M,K,length(Rician_Region));
...
%Go through all UEs
 for k = 1:K
     ...
     for ii = 1:length(Rician_Region)

         d_Rician_Threshold = Rician_Region(ii);
         probLoS = double(distances<d_Rician_Threshold);

         %Calculate the distance based Rician Factor
         

         %Save the channel gains (in this setup each pair has a LoS path)
         channelGaindB(probLoS==1,k,ii)=-betaLoS(probLoS==1);
         channelGaindB(probLoS==0,k,ii)=-betaNLoS(probLoS==0);
         channelGaindB(:,k,ii) = channelGaindB(:,k,ii)+shadowFading(:,k)-noiseVariancedBm+30;
         channelGain(:,k,ii) = db2pow(channelGaindB(:,k,ii));


         %Scale with Rician factor
         channelGain_LoS(probLoS==1,k,ii,n) = sqrt(RicianFactor(probLoS==1,k)./(RicianFactor(probLoS==1,k) +1 )).*sqrt(channelGain(probLoS==1,k));
         channelGain_NLoS(probLoS==1,k,ii,n) = (1./(RicianFactor(probLoS==1,k) +1 )).*(channelGain(probLoS==1,k));
         channelGain_NLoS(probLoS==0,k,ii,n) = channelGain(probLoS==0,k); %note that probLoS is always one in the manuscript
         ...
         for m = 1:M
             ...
             %scattering model
             R_AP(:,:,m,k,ii,n) = channelGain_NLoS(m,k,ii,n)*functionRlocalscattering(N,angletoUE,ASDdeg,antennaSpacing);

             %Generate HMean_Withoutphase in Multi-Antenna case
             HMean_Withoutphase((m-1)*N+1:m*N,k,ii,n) = channelGain_LoS(m,k,ii,n)*(exp(1i*2*pi.*(0:(N-1))*sin(angletoUE)*antennaSpacing));
