function [SE] = functionComputeSE_Distributed_Analytical_wo_PS(W,G_LoS_eff,Rhat,R,Phi,tau_c,tau_p,Pset,N,K,M,p)
%%=============================================================
...
for k = 1:K
    for m = 1:M
        ...
        denominator3(k) = denominator3(k) + trace(W(:,:,m,k)'*W(:,:,m,k)*Gbar_mkl(:,:,m,k,k)) + trace(W(:,:,m,k)'*W(:,:,m,k)*Rhat(:,:,m,k));

        for l = 1:K

            mu_mkl(k) = mu_mkl(k) + p(l)*trace(W(:,:,m,k)'*Gbar_mkl(:,:,m,l,l)*W(:,:,m,k)*Gbar_mkl(:,:,m,k,k)) + p(l)*trace(W(:,:,m,k)'*R(:,:,m,l)*W(:,:,m,k)*Gbar_mkl(:,:,m,k,k))...
                + p(l)*trace(W(:,:,m,k)'*Gbar_mkl(:,:,m,l,l)*W(:,:,m,k)*Rhat(:,:,m,k)) + p(l)*trace(W(:,:,m,k)'*R(:,:,m,l)*W(:,:,m,k)*Rhat(:,:,m,k));

            gamma(k,l,m) = trace(W(:,:,m,k)'*Gbar_mkl(:,:,m,l,k));


            if any(l == Pset(:,k))

                mu_mkl(k) = mu_mkl(k) + p(l)*sqrt(p(k)*p(l))*tau_p*trace(W(:,:,m,k)'*Gbar_mkl(:,:,m,l,k))*trace(W(:,:,m,k)*Phi_cross_mlk(:,:,m,k,l)) + p(l)*sqrt(p(k)*p(l))*tau_p*trace(W(:,:,m,k)'*Phi_cross_mlk(:,:,m,l,k))*trace(W(:,:,m,k)*Gbar_mkl(:,:,m,k,l))...
