function [C_LS,C_total,C_total_blk] = functionMatrixGeneration_LS(R,HMean_Withoutphase,p,M,K,N,tau_p,Pset)
%%=============================================================
%The file is used to generate the matrices based on the LS channel estimator used in the next subsequent calculation of the paper:
%
...
for m = 1:M
    ...
    for k = 1:K
        ...
        for z = 1:length(inds)   
            ...
            PsiInv_LS = PsiInv_LS + p(inds(z))*tau_p*Rp(:,:,m,inds(z));

        end
            PsiInv_LS = PsiInv_LS + eye(N);

           
            
            RHat(:,:,m,k) = 1/(p(k)*tau_p)*PsiInv_LS;
