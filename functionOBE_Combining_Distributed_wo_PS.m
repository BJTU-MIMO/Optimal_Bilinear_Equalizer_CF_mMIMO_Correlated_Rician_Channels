function [V_OBE_Combining_Distributed,W_OBE_matrix] = functionOBE_Combining_Distributed_wo_PS(G_LoS_eff,Ghat_MMSE,Rhat,Phi,R,Pset,M,N,K,p,tau_p,nbrOfRealizations)
%%=============================================================
%The file is used to design DG-OBE combining vectors without random phase-shifts based on closed-form OBE matrices of the paper:
%
...
for k = 1:K
    for m = 1:M
        for l = 1:K
            ...
            Lambda_k_5((m-1)*N^2+1:m*N^2,(m-1)*N^2+1:m*N^2,k) = kron(Gbar_mkk.',eye(N)) + kron(Rhat(:,:,m,k).',eye(N));
    ...
    for l = 1:K
        ...
        for m = 1:M
            ...
            Lambda_kl_1((m-1)*N^2+1:m*N^2,(m-1)*N^2+1:m*N^2,k,l) = p(l)*kron(Gbar_mkl(:,:,m,k,k).',Gbar_mkl(:,:,m,l,l)) + p(l)*kron(Gbar_mkl(:,:,m,k,k).',R(:,:,m,l))...
                + p(l)*kron(Rhat(:,:,m,k).',Gbar_mkl(:,:,m,l,l)) + p(l)*kron(Rhat(:,:,m,k).',R(:,:,m,l));
            ...
            Lambda_kl_2((m-1)*N^2+1:m*N^2,(m-1)*N^2+1:m*N^2,k,l) = p(l)*(sqrt(p(k)*p(l))*tau_p*gbar_mlk*rwave_mlk' + sqrt(p(k)*p(l))*tau_p*rwave_mlk*gbar_mlk' + p(k)*p(l)*tau_p^2*(rwave_mlk*rwave_mlk'));
...
clear Gbar_mkl Bbar_lk_blk Bhat_lk B_mlk Rbar_mk Phi_cross_mlk Gbar_lk_2 Gbarbar_lk_blk
